...
trait ReleaseNotesTasks {
  ...
  public function generateReleaseNotes(?string $tag = NULL) {
    ...
    [$githubOrg, $githubProject] = $this->detectGitHubProject();

    if (empty($githubOrg) || empty($githubProject)) {
    ...
    $releaseData = $this->fetchReleaseData($githubOrg, $githubProject, $prNumbers);
